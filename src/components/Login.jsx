  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (err.code === 'auth/popup-closed-by-user' || err.code === 'auth/cancelled-popup-request') {
        ...
        setError('Dominio no autorizado. Agrega este dominio en Firebase Console → Authentication → Authorized domains.');
      } else {
      ...
      {currentUser ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
          <p style={{ color: '#888', fontSize: 13, margin: 0 }}>
            Sesión activa como <strong style={{ color: '#fff' }}>{currentUser.email}</strong>
          </p>
          <p style={{ color: '#f87171', fontSize: 13, margin: 0, textAlign: 'center', maxWidth: 300 }}>
            Esta cuenta no tiene acceso. Cierra sesión e inicia con {allowedEmail}.
          </p>
          <button onClick={handleSignOut} style={{
            padding: '10px 20px', borderRadius: 8, border: '1px solid #444',
            background: '#1a1a1a', color: '#f87171', fontSize: 14,
